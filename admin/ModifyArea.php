...
$adminUtil->CheckAdminPagePrivs($globalUtil,$_SESSION['adminUserSession']['uid'],$_SESSION['adminUserSession']['privs'],"Modify Area");
...
if($doaction=="add"){
	$pageHeading="Add Area";
	$submitValue="Submit";
	}
elseif($doaction=="edit"){
	
	$id=$_GET['id'];
	$pageHeading="Modify Area";
	$submitValue="Update";
	
	
	$formData=$areaObj->getAreas($globalUtil,"WHERE area.id='".$id."'");
			...
			<table width="100%" cellpadding="0" cellspacing="0" id="InnerTable" class="InnerTable">
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
						<form id="adminformmain" name="adminformmain" action="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL.CONTROLLER_FOLDER.CONTROLLER_Area);?>" enctype="multipart/form-data" method="post" class="formstyle1">
                        <?php
                        if($doaction=="edit"){
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            <?php
							if($msgInfo->hasMsg('modifyareamsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('modifyareamsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
							...
							?>
                            <tr>
								<td class="formlabel">Area Name</td>
                                <td class="separter">:</td>
                                <td class="forminput"><div class="forminputboxbg"><input type="input" class="formtextbox" id="areaName" name="areaName" validate="{required:true,messages:{required:'Please enter an area name.'}}" value="<?php echo $adminUtil->getPostFormData("areaName");?>"/></div></td>
							</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>  
                            <tr>
                                ...
                                <td class="separter">:</td>
                                <td class="forminput"><?php print $areaObj->getCountryOptions($globalUtil,array("id"=>'cid',"name"=>'cid','validate'=>"{required:true,messages:{required:'Select a country.'}}"),$adminUtil->getPostFormData("cid"));?></td>
							</tr>      
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr> 
                            <tr>
                                ...
                                <td class="separter">:</td>
                                <td class="forminput"><?php print $areaObj->getStateOptions($globalUtil,array("id"=>'sid',"name"=>'sid','validate'=>"{required:true,messages:{required:'Select a state.'}}"),$adminUtil->getPostFormData("sid"),"WHERE status='1' AND cid='".$adminUtil->getPostFormData("cid")."'");?></td>
							</tr>      
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr> 
