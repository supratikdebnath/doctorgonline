...
if($doaction=="add"){
	$pageHeading="Add Insurance TPA";
	$submitValue="Submit";
	}
elseif($doaction=="edit"){
	
	$id=$_GET['id'];
	$pageHeading="Modify Insurance TPA";
	$submitValue="Update";
	
	$insuranceTPAObj=new InsuranceTPA;
	$formData=$insuranceTPAObj->getInsuranceTPA($globalUtil,"WHERE id='".$id."'");
			...
			<table width="100%" cellpadding="0" cellspacing="0" id="InnerTable" class="InnerTable">
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
						<form id="adminformmain" name="adminformmain" action="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL.CONTROLLER_FOLDER.CONTROLLER_InsuranceTPA);?>" enctype="multipart/form-data" method="post" class="formstyle1">
                        <?php
                        if($doaction=="edit"){
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            <?php
							if($msgInfo->hasMsg('modifyInsuranceTPAmsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('modifyInsuranceTPAmsg');?></td>
                    		</tr>
                            <tr>
                           	<td colspan="3" height="5px">                            </tr>
                   		    <?php
							...
							?>
                            <tr>
								<td class="formlabel"> Insurance TPA Name</td>
                                <td class="separter">:</td>
                                <td class="forminput"><div class="forminputboxbg"><input type="input" class="formtextbox" id="insurance_company" name="insurance_tpa" validate="{required:true,messages:{required:'Please enter a Insurance TPA.'}}" value="<?php echo $adminUtil->getPostFormData("insurance_tpa");?>"/></div></td>
							</tr>
                            <tr>
                           	<td colspan="3" height="5px">                            </tr>      
                            <tr>
