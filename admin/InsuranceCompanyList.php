...
require(ADMIN_DOCUMENT_PATH.CONTROLLER_FOLDER.CONTROLLER_InsuranceCompanyList);
...
function delRecord(val,id)
{
	if(val=='0')
	{
		...
		{
			window.location.href='<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."InsuranceCompanyList.php")."?do=del&id=";?>'+id;
		}
	}
	else if(val!='0')
	{
				...
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0"class="formstyle1">
							<tr>
							  <td class="PageHeading" colspan="3"><span class="listHeading">Insurance Company List</span></td>
							</tr>
							<tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            <?php
							if($msgInfo->hasMsg('listInsuranceCompanymsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('listInsuranceCompanymsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            ...
                            <tr>
							    ...
							    <table width="100%" cellpadding="4" cellspacing="0" id="DataTable">
                                	<tr>
                                        <td class="listHeading">Insurance Company</td>
                                    ...
                                    <?php
                                    if($allInsuranceCompany['numrows']>0){
									for($i=0;$i<$allInsuranceCompany['numrows'];$i++){
									?>
                                    <tr>
                                        <td class="listData"><?php echo $allInsuranceCompany['data'][$i]['insurance_company'];?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allInsuranceCompany['data'][$i]['createDt'],'d-M-Y');?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allInsuranceCompany['data'][$i]['modifyDt'],'d-M-Y');?></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."InsuranceCompanyList.php")."?do=status&id=".$allInsuranceCompany['data'][$i]['id'];?>"><?php echo $globalUtil->getStatusMessage($allInsuranceCompany['data'][$i]['status']);?></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."ModifyInsuranceCompany.php")."?do=edit&id=".$allInsuranceCompany['data'][$i]['id']."&returnurl=".$globalUtil->base64encode($globalUtil->currentUrl());?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-edit.png" class="icon1"/></a></td>
                                        <td class="listData"><a href="javascript:void(0);" onclick="delRecord('<?php echo $allInsuranceCompany['data'][$i]['status'];?>','<?php echo $allInsuranceCompany['data'][$i]['id']; ?>')"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-delete.png" class="icon1"/></a></td>
									</tr>
                                	<?php }
									}
									else{
