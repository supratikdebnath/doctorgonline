...
$adminUtil->CheckAdminPagePrivs($globalUtil,$_SESSION['adminUserSession']['uid'],$_SESSION['adminUserSession']['privs'],"Modify City");
...
if($doaction=="add"){
	$pageHeading="Add City";
	$submitValue="Submit";
	}
elseif($doaction=="edit"){
	
	$id=$_GET['id'];
	$pageHeading="Modify City";
	$submitValue="Update";
	
	
	$formData=$cityObj->getCity($globalUtil,"WHERE city.id='".$id."'");
			...
			<table width="100%" cellpadding="0" cellspacing="0" id="InnerTable" class="InnerTable">
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
						<form id="adminformmain" name="adminformmain" action="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL.CONTROLLER_FOLDER.CONTROLLER_City);?>" enctype="multipart/form-data" method="post" class="formstyle1">
                        <?php
                        if($doaction=="edit"){
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            <?php
							if($msgInfo->hasMsg('modifycitymsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('modifycitymsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
							...
							?>
                            <tr>
								<td class="formlabel">City Name</td>
                                <td class="separter">:</td>
                                <td class="forminput"><div class="forminputboxbg"><input type="input" class="formtextbox" id="cityName" name="cityName" validate="{required:true,messages:{required:'Please enter a city name.'}}" value="<?php echo $adminUtil->getPostFormData("cityName");?>"/></div></td>
							</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>  
                            <tr>
                                ...
                                <td class="separter">:</td>
                                <td class="forminput"><?php print $cityObj->getCountryOptions($globalUtil,array("id"=>'cid',"name"=>'cid','validate'=>"{required:true,messages:{required:'Select a country.'}}"),$adminUtil->getPostFormData("cid"));?></td>
							</tr>      
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr> 
                            <tr>
                                ...
                                <td class="separter">:</td>
                                <td class="forminput"><?php print $cityObj->getStateOptions($globalUtil,array("id"=>'sid',"name"=>'sid','validate'=>"{required:true,messages:{required:'Select a state.'}}"),$adminUtil->getPostFormData("sid"),"WHERE status='1' AND cid='".$adminUtil->getPostFormData("cid")."'");?></td>
							</tr>      
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr> 
                            <tr>
								<td class="formlabel">Zone</td>
                                <td class="separter">:</td>
                                <td class="forminput"><?php print $cityObj->getZoneOptions($globalUtil,array("id"=>'zid',"name"=>'zid','validate'=>"{required:true,messages:{required:'Select a zone.'}}"),$adminUtil->getPostFormData("zid"),"WHERE status='1'");?></td>
							</tr>      
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>                                                
