...
require(ADMIN_DOCUMENT_PATH.CONTROLLER_FOLDER.CONTROLLER_StateList);
$adminUtil->CheckAdminPagePrivs($globalUtil,$_SESSION['adminUserSession']['uid'],$_SESSION['adminUserSession']['privs'],"State List");
				...
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0"class="formstyle1">
							<tr><td class="PageHeading" colspan="3">State List</td></tr>
							<tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            <?php
							if($msgInfo->hasMsg('liststatemsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('liststatemsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            ...
                            <tr>
							    ...
							    <table width="100%" cellpadding="4" cellspacing="0" id="DataTable">
                                    ...
                                    <?php
                                    if($allStates['numrows']>0){
									for($i=0;$i<$allStates['numrows'];$i++){
									?>
                                    <tr>
                                        <td class="listData"><?php echo $allStates['data'][$i]['stateName'];?></td>
                                        <td class="listData"><?php echo $allStates['data'][$i]['countryName'];?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allStates['data'][$i]['createDt'],'d-M-Y');?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allStates['data'][$i]['modifyDt'],'d-M-Y');?></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."StateList.php")."?do=status&id=".$allStates['data'][$i]['id'];?>"><?php echo $globalUtil->getStatusMessage($allStates['data'][$i]['status']);?></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."ModifyState.php")."?do=edit&id=".$allStates['data'][$i]['id']."&returnurl=".$globalUtil->base64encode($globalUtil->currentUrl());?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-edit.png" class="icon1"/></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."StateList.php")."?do=del&id=".$allStates['data'][$i]['id'];?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-delete.png" class="icon1"/></a></td>
									</tr>
                                	<?php }
									}
									else{
									?>
                                    <tr>
                                        <td class="listData" align="center" colspan="7">No Records Found.</td>
									</tr>
                                    <?php
									}
									?>
