...
require(ADMIN_DOCUMENT_PATH.CONTROLLER_FOLDER.CONTROLLER_CityList);
$adminUtil->CheckAdminPagePrivs($globalUtil,$_SESSION['adminUserSession']['uid'],$_SESSION['adminUserSession']['privs'],"City List");
				...
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0"class="formstyle1">
                            ...
                            <?php
							if($msgInfo->hasMsg('listcitymsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('listcitymsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            ...
                            <tr>
							    ...
							    <table width="100%" cellpadding="4" cellspacing="0" id="DataTable">
                                    ...
                                    <?php
                                    if($allCity['numrows']>0){
									for($i=0;$i<$allCity['numrows'];$i++){
									?>
                                    <tr>
                                        <td class="listData"><?php echo $allCity['data'][$i]['cityName'];?></td>
                                        <td class="listData"><?php echo $allCity['data'][$i]['countryName'];?></td>
                                        <td class="listData"><?php echo $allCity['data'][$i]['stateName'];?></td>
                                        <td class="listData"><?php echo $allCity['data'][$i]['zoneName'];?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allCity['data'][$i]['createDt'],'d-M-Y');?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allCity['data'][$i]['modifyDt'],'d-M-Y');?></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."CityList.php")."?do=status&id=".$allCity['data'][$i]['id'];?>"><?php echo $globalUtil->getStatusMessage($allCity['data'][$i]['status']);?></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."ModifyCity.php")."?do=edit&id=".$allCity['data'][$i]['id']."&returnurl=".$globalUtil->base64encode($globalUtil->currentUrl());?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-edit.png" class="icon1"/></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."CityList.php")."?do=del&id=".$allCity['data'][$i]['id'];?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-delete.png" class="icon1"/></a></td>
									</tr>
                                	<?php }
									}
									else{
