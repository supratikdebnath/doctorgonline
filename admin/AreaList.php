...
require(ADMIN_DOCUMENT_PATH.CONTROLLER_FOLDER.CONTROLLER_AreaList);
$adminUtil->CheckAdminPagePrivs($globalUtil,$_SESSION['adminUserSession']['uid'],$_SESSION['adminUserSession']['privs'],"Area List");
				...
				<tr>
					...
					<td id="rightpanel" class="rightpanel">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0"class="formstyle1">
							<tr><td class="PageHeading" colspan="3">City List</td></tr>
							<tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            <?php
							if($msgInfo->hasMsg('listareamsg')){
							?>
                    		<tr>
                    			<td class="msgposition" colspan="3"><?php echo $msgInfo->displayMsg('listareamsg');?></td>
                    		</tr>
                            <tr>
                            	<td colspan="3" height="5px">
                            </tr>
                            ...
                            <tr>
							    ...
							    <table width="100%" cellpadding="4" cellspacing="0" id="DataTable">
                                    ...
                                    <?php
                                    if($allArea['numrows']>0){
									for($i=0;$i<$allArea['numrows'];$i++){
									?>
                                    <tr>
                                        <td class="listData"><?php echo $allArea['data'][$i]['areaName'];?></td>
                                        <td class="listData"><?php echo $allArea['data'][$i]['countryName'];?></td>
                                        <td class="listData"><?php echo $allArea['data'][$i]['stateName'];?></td>
                                        <td class="listData"><?php echo $allArea['data'][$i]['zoneName'];?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allArea['data'][$i]['createDt'],'d-M-Y');?></td>
                                        <td class="listData"><?php echo $globalUtil->dateFromTime($allArea['data'][$i]['modifyDt'],'d-M-Y');?></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."AreaList.php")."?do=status&id=".$allArea['data'][$i]['id'];?>"><?php echo $globalUtil->getStatusMessage($allArea['data'][$i]['status']);?></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."ModifyArea.php")."?do=edit&id=".$allArea['data'][$i]['id']."&returnurl=".$globalUtil->base64encode($globalUtil->currentUrl());?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-edit.png" class="icon1"/></a></td>
                                        <td class="listData"><a href="<?php echo $globalUtil->generateUrl(ADMIN_SITE_URL."AreaList.php")."?do=del&id=".$allArea['data'][$i]['id'];?>"><img src="<?php echo ADMIN_IMAGES_URL;?>icon-delete.png" class="icon1"/></a></td>
									</tr>
                                	<?php }
									}
									else{
									?>
                                    <tr>
                                        <td class="listData" align="center" colspan="9">No Records Found.</td>
									</tr>
                                    <?php
									}
									?>
